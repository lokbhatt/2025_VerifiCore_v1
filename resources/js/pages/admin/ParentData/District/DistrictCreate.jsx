...
const DistrictCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res= await api.post("/parent-data/districts", formData) 
      navigate("/admin/parent-data/district");
    } catch (error) {
      if (error.response?.status === 422) {
        setErrors(error.response.data.errors || error.response.data.data || {});
      } else {
        console.log("Failed to create district. Please try again.");
      }
    } finally {
  ...
  return (
    <div className="create-page px-6 py-8 max-w-4xl mx-auto">
      <header className="flex items-center justify-around mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Add New District</h2>
          <Link to="/admin/parent-data/district" className="btn btn-secondary bg-gray-600 hover:bg-gray-700 text-white text-sm px-4 py-2 rounded transition">
            Back to Index
          </Link>
      </header>
      <MainForm
